...
export async function GET(request: Request) {
  try {
    ...
    const { isAdmin, isModerator } = await ensureAppUser(auth.authUserId, auth.email);
    ...
    const { data, error } = await supabase
      ...
      .select("id, email, message, status, created_at, reviewed_at, review_note")
    ...
    return NextResponse.json({ inviteRequests: data ?? [] });
  } catch (error) {
